...
import { getFollowing, followUser } from '../../apis/userApi';
...
// ─── User Row ──────────────────────────────────────────────────────────────
const UserRow = ({ user, onPress, onUnfollow }) => {
  ...
  return (
    ...
    >
      ...
      {!isMe && (
        ...
        >
          {loading ? (
            <ActivityIndicator size="small" color={C.textOff} />
          ) : (
            <View style={styles.followingBtnInner}>
              <Ionicons name="checkmark" size={14} color={C.textOff} />
...
const FollowingScreen = () => {
  ...
  const fetchFollowing = useCallback(async (pageNum = 1, shouldRefresh = false) => {
    try {
      if (shouldRefresh) setRefreshing(true);
      else if (pageNum === 1) setLoading(true);
      else setLoadingMore(true);

      const res = await getFollowing(targetUserId, { page: pageNum, limit: 20 });
  ...
  useEffect(() => { fetchFollowing(1); }, [targetUserId]);

  const handleRefresh = () => fetchFollowing(1, true);
  const handleLoadMore = () => {
    if (hasMore && !loadingMore && !loading) fetchFollowing(page + 1);
  };
  ...
  const handleUnfollow = (unfollowedId) => {
    setFollowing(prev => prev.filter(u => u._id !== unfollowedId));
    ...
    <View>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          ...
          {total > 0 && (
            <Text style={styles.headerCount}>{total} {total === 1 ? 'person' : 'people'}</Text>
          )}
        </View>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptySubtitle}>
          {targetUserId === currentUser?._id
            ? 'Follow people to see their posts in your feed. Explore the campus community!'
        ...
        {targetUserId === currentUser?._id && (
          ...
          >
            ...
            <Text style={styles.exploreBtnText}>Explore Campus Feed</Text>
          </TouchableOpacity>
        )}
      </View>
    );
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <FlatList
        ...
        contentContainerStyle={[
          styles.listContent,
          following.length === 0 && !loading && styles.listContentEmpty,
        ]}
...
const styles = StyleSheet.create({
  ...
  bioText: { fontSize: 12, color: C.textMuted, marginTop: 2 },

  // Following Button (different from Followers — shows "Following" state)
  followingBtn: {
    backgroundColor: C.brandDim,
    paddingHorizontal: 14, paddingVertical: 8,
    borderRadius: 20, borderWidth: 1, borderColor: C.border,
    minWidth: 105, alignItems: 'center',
  },
  followingBtnInner: {
    flexDirection: 'row', alignItems: 'center', gap: 4,
  },
  followingBtnText: { color: C.textOff, fontSize: 12.5, fontWeight: '600' },
