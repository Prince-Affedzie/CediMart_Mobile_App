...
const VendorLoginScreen = ({ navigation }) => {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      else { Alert.alert('Error', response?.data?.message || 'Failed to send verification code. Please try again.'); }
    } catch (err) { Alert.alert('Error', err?.response?.data?.message || err?.response?.data?.error || 'Network error. Please check your connection.'); }
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      if (response?.data?.success || response?.status === 200) {
        ...
        if (vendorRes.success) { setStep(STEPS.PHONE); setOtp(''); }
        else { Alert.alert('Login Failed', vendorRes?.error || vendorRes?.message || "You haven't created a vendor account with us."); setStep(STEPS.PHONE); setOtp(''); }
      } else { setError('Invalid or expired code. Please try again.'); }
    } catch (err) { setError(err?.response?.data?.message || err?.response?.data?.error || 'Verification failed. Please try again.'); }
    finally { setLoading(false); }
  };
  ...
  const handleResendOTP = async () => {
    ...
    try {
      const trimmed = phone.trim().replace(/\s/g, '');
      const response = await getOTP(trimmed);
      if (response?.data?.success || response?.status === 200) { setResendCooldown(180); Alert.alert('Code Sent', 'A new verification code has been sent to your phone.'); }
      else { Alert.alert('Error', 'Failed to resend code. Please try again.'); }
      ...
      <View style={styles.form}>
        <View style={styles.inputGroup}>
          ...
          <Animated.View style={[styles.inputContainer, error && styles.inputError, { transform: [{ translateX: shakeAnim }] }]}>
            ...
            <TextInput style={styles.input} placeholder="Enter phone number" placeholderTextColor={C.t3} value={phone} onChangeText={(text) => { setError(''); setPhone(text.replace(/[^0-9]/g, '')); }} keyboardType="phone-pad" maxLength={10} editable={!loading} returnKeyType="done" onSubmitEditing={handleSendOTP} />
          </Animated.View>
          {error !== '' && (
            <View style={styles.errorRow}><Ionicons name="alert-circle-outline" size={14} color={C.danger} /><Text style={styles.errorText}>{error}</Text></View>
          )}
        </View>
        <TouchableOpacity style={[styles.primaryButton, loading && styles.primaryButtonDisabled]} onPress={handleSendOTP} disabled={loading} activeOpacity={0.8}>
          {loading ? <ActivityIndicator size="small" color="#fff" /> : <><Text style={styles.primaryButtonText}>Login</Text><Ionicons name="arrow-forward" size={20} color="#fff" /></>}
        </TouchableOpacity>
        <View style={styles.linkRow}>
          <TouchableOpacity onPress={() => navigation.navigate('VendorSignUp')} disabled={loading}>
            <Text style={[styles.linkAction, loading && styles.disabledText]}>Don't have Vendor Account?</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Animated.View>
  ...
  const renderOTPStep = () => (
    <Animated.View style={{ transform: [{ translateY: slideAnim }] }}>
      ...
      {error !== '' && (
        <View style={[styles.errorRow, { justifyContent: 'center', marginTop: 10 }]}><Ionicons name="alert-circle-outline" size={14} color={C.danger} /><Text style={styles.errorText}>{error}</Text></View>
      )}
      <TouchableOpacity style={[styles.primaryButton, (loading || otp.length < 6) && styles.primaryButtonDisabled]} onPress={handleVerifyOTP} disabled={loading || otp.length < 6} activeOpacity={0.8}>
        {loading ? <ActivityIndicator size="small" color="#fff" /> : <><Text style={styles.primaryButtonText}>Verify & Sign In</Text><Ionicons name="checkmark-circle-outline" size={20} color="#fff" /></>}
      </TouchableOpacity>
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <View style={styles.topBar}>
          <TouchableOpacity style={styles.backButton} onPress={() => { if (step === STEPS.PHONE) navigation.goBack(); else { setStep(STEPS.PHONE); setOtp(''); setError(''); } }} disabled={loading}>
        ...
        <LoadingOverlay visible={loading} message={step === STEPS.PHONE ? 'Sending verification code...' : 'Verifying your phone...'} />
...
const styles = StyleSheet.create({
  ...
  header: { paddingHorizontal: 20, paddingTop: 40, paddingBottom: 30, alignItems: 'center' },
  logoContainer: { flexDirection: 'row', alignItems: 'center', marginBottom: 16 },
  brandLogo: { width: 60, height: 60, marginRight: 8 },
  title: { fontSize: 28, fontWeight: 'bold', color: C.brand, marginBottom: 8 },
  subtitle: { fontSize: 16, color: C.t2, textAlign: 'center' },
  form: { paddingHorizontal: 20 },
  inputGroup: { marginBottom: 20 },
  label: { fontSize: 14, fontWeight: '600', color: '#333', marginBottom: 8 },
  inputContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: C.gray100, borderRadius: 12, borderWidth: 1, borderColor: C.gray200, paddingHorizontal: 16, overflow: 'hidden' },
  ...
  primaryButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: C.brand, borderRadius: 12, paddingVertical: 16, marginBottom: 14, gap: 8, shadowColor: C.brand, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.25, shadowRadius: 8, elevation: 4 },
  ...
  stepHeader: { alignItems: 'center', marginBottom: 32, marginTop: 40, paddingHorizontal: 20 },
  ...
  otpBox: { width: 48, height: 56, borderRadius: 12, borderWidth: 2, borderColor: '#E0E0E0', backgroundColor: C.gray100, justifyContent: 'center', alignItems: 'center' },
