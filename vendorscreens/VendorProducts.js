...
const StatCard = ({ value, label, valueColor, icon, onPress, alert }) => (
  ...
  >
    <View style={styles.statCardTop}>
      <View style={[styles.statIconWrap, { backgroundColor: (valueColor || '#2E7D32') + '18' }]}>
        <Ionicons name={icon} size={15} color={valueColor || '#2E7D32'} />
      </View>
      {alert ? (
        <View style={styles.statAlertDot} />
      ) : null}
    </View>
    <Text style={[styles.statValue, { color: valueColor || '#1A1A1A' }]}>{value}</Text>
      ...
      <View style={styles.cardBody}>
        {condition && (
          <View style={[styles.conditionPill, { backgroundColor: condition.bg }]}>
            <Text style={[styles.conditionPillText, { color: condition.color }]}>{condition.label}</Text>
          </View>
        )}
...
const MyProductsScreen = ({ navigation }) => {
  ...
  const handleToggleAvailability = (product) => {
    ...
    Alert.alert(
      ...
      newState ? `"${product.name}" will appear in search results.` : `"${product.name}" will be hidden from buyers.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Confirm',
    ...
    <View>
        ...
        <SafeAreaView edges={['top']}>
          <View style={styles.heroInner}>
            ...
            <View style={styles.heroActions}>
              <TouchableOpacity style={styles.heroIconBtn} onPress={() => setShowSortModal(true)}>
                <Ionicons name="swap-vertical-outline" size={18} color="#fff" />
              </TouchableOpacity>
              <TouchableOpacity style={styles.heroIconBtn} onPress={() => fetchProducts(true)}>
      ...
      <View style={styles.statsStrip}>
        ...
        <StatCard
          ...
          label="Low Stock"
          valueColor={stats.lowStock > 0 ? '#E65100' : '#9E9E9E'}
          onPress={stats.lowStock > 0 ? () => setActiveTab('low') : null}
        ...
        <StatCard
          ...
          value={stats.soldOut}
          label="Sold Out"
          valueColor={stats.soldOut > 0 ? '#C62828' : '#9E9E9E'}
          onPress={stats.soldOut > 0 ? () => setActiveTab('sold') : null}
        />
      </View>
        ...
        {TABS.map(tab => {
          ...
          return (
            <TouchableOpacity
              ...
              activeOpacity={0.75}
            >
              <Ionicons name={tab.icon} size={13} color={isActive ? '#fff' : '#9E9E9E'} />
      ...
      {stats.lowStock > 0 && activeTab !== 'low' && (
        <TouchableOpacity style={styles.alertBanner} onPress={() => setActiveTab('low')} activeOpacity={0.85}>
    ...
    return (
      <View style={styles.emptyWrap}>
        <View style={styles.emptyIconCircle}>
          <Ionicons name={cfg.icon} size={34} color="#A5D6A7" />
        </View>
        <Text style={styles.emptyTitle}>{cfg.title}</Text>
        <Text style={styles.emptySub}>{cfg.sub}</Text>
        {!isSearching && activeTab === 'all' && (
          <TouchableOpacity style={styles.emptyAddBtn} onPress={() => navigation.navigate('AddProduct')}>
  ...
  return (
    <View style={styles.container}>
      ...
      {/* FAB */}
      <TouchableOpacity style={styles.fab} onPress={() => navigation.navigate('AddProduct')} activeOpacity={0.88}>
...
const styles = StyleSheet.create({
  ...
  heroDecor1: {
    ...
    backgroundColor: 'rgba(255,255,255,0.05)', right: -40, top: -50,
  },
  heroDecor2: {
    position: 'absolute', width: 90, height: 90, borderRadius: 45,
    backgroundColor: 'rgba(255,255,255,0.04)', right: 80, bottom: -10,
  },
  heroInner: {
  ...
  // Stats strip — card row that sits just below the hero
  statsStrip: {
    flexDirection: 'row',
    backgroundColor: '#fff',
    marginHorizontal: 14,
    marginTop: -1,           // flush under hero
  ...
  statLabel: { fontSize: 10, color: '#9E9E9E', fontWeight: '600', marginTop: 2, textAlign: 'center' },
  statDivider: { width: 1, height: 40, backgroundColor: '#F0F0F0', alignSelf: 'center' },

  // Search — own row, outside the hero
  ...
  imgCountBadge: {
    ...
    flexDirection: 'row', alignItems: 'center', gap: 3,
    backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: 8, paddingHorizontal: 6, paddingVertical: 3,
  },
  imgCountText: { fontSize: 9, fontWeight: '700', color: '#fff' },
  negotiableBadge: {
    position: 'absolute', top: 7, left: 7,
    backgroundColor: 'rgba(249,115,22,0.92)', borderRadius: 8, paddingHorizontal: 6, paddingVertical: 3,
  },
  ...
  cardName: { fontSize: 13, fontWeight: '700', color: '#1B2714', lineHeight: 17, marginBottom: 2, minHeight: 34 },
  cardCategory: { fontSize: 10, fontWeight: '700', color: '#BDBDBD', textTransform: 'uppercase', letterSpacing: 0.4, marginBottom: 5 },
  ...
  editBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 3,
    backgroundColor: '#E8F5E9', borderRadius: 9, paddingHorizontal: 9, paddingVertical: 5,
  ...
  emptyIconCircle: { width: 82, height: 82, borderRadius: 41, backgroundColor: '#E8F5E9', justifyContent: 'center', alignItems: 'center', marginBottom: 18 },
  emptyTitle: { fontSize: 19, fontWeight: '800', color: '#1B2714', marginBottom: 8 },
  emptySub: { fontSize: 13, color: '#9E9E9E', textAlign: 'center', lineHeight: 20, marginBottom: 24 },
  emptyAddBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 7,
    backgroundColor: '#2E7D32', paddingHorizontal: 22, paddingVertical: 12, borderRadius: 14,
  ...
  sortSheet: {
    ...
    shadowColor: '#000', shadowOffset: { width: 0, height: -4 }, shadowOpacity: 0.1, shadowRadius: 12, elevation: 18,
  },
  ...
  fab: {
    ...
    justifyContent: 'center', alignItems: 'center',
    shadowColor: '#1B5E20', shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.38, shadowRadius: 10, elevation: 8,
  },
  hintBar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, paddingVertical: 8, backgroundColor: '#F2F4F0' },
