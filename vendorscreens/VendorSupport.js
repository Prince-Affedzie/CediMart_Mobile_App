...
import {
  ...
  Linking, Alert, StatusBar, Animated, Dimensions,
} from 'react-native';
...
const VendorSupportScreen = ({ navigation }) => {
  ...
  const faqs = [
    ...
    { id: 8, question: 'Why is my store showing as "Pending Verification"?', answer: 'All new vendor accounts go through a verification process to ensure trust and safety on the platform. This usually takes 24-48 hours. Once verified, you\'ll receive a notification and your store will show as "Verified" — which increases buyer confidence.', category: 'account' },
    { id: 9, question: 'Can I sell on multiple campuses?', answer: 'Currently, your store is linked to one primary campus. However, students from other campuses can still discover your products through search. We recommend setting your campus to where you\'re physically located for easier delivery logistics.', category: 'selling' },
    { id: 10, question: 'How do I handle a dispute with a buyer?', answer: 'If there\'s a disagreement with a buyer, contact our support team immediately via WhatsApp or phone. We\'ll mediate the situation fairly, reviewing product descriptions, photos, and communication history to reach a resolution.', category: 'general' },
    { id: 11, question: 'What products are prohibited from sale?', answer: 'We do not allow the sale of counterfeit goods, stolen items, weapons, alcohol, drugs, or any items that violate university policies or Ghanaian law. Violations may result in account suspension.', category: 'general' },
    { id: 12, question: 'How do I boost my product visibility?', answer: 'Use clear, well-lit photos, write detailed descriptions, set competitive prices, and select relevant categories and tags like "Featured" or "Student Favorite". Products with better ratings and more views naturally rank higher.', category: 'selling' },
  ];
  ...
  const handleEmail = () => Linking.openURL('mailto:vendors@cedimart.com?subject=Cedimart Vendor Support');
  const handleWhatsApp = () => Linking.openURL('https://wa.me/233505671577?text=Hello%20Cedimart%20Vendor%20Support');
  const handleFAQToggle = (id) => setActiveFAQ(activeFAQ === id ? null : id);
  const filteredFAQs = selectedTopic === 'all' ? faqs : faqs.filter((faq) => faq.category === selectedTopic);

  const channels = [
    { id: 'whatsapp', label: 'WhatsApp', sub: 'Fastest response', icon: 'logo-whatsapp', color: '#25D366', bg: '#F0FDF4', onPress: handleWhatsApp },
    { id: 'call', label: 'Call Us', sub: '+233 50 567 1577', icon: 'call', color: C.info, bg: C.infoBg, onPress: () => handleCall('+233505671577') },
    { id: 'email', label: 'Email', sub: 'Within 2 hours', icon: 'mail', color: C.danger, bg: C.dangerBg, onPress: handleEmail },
  ];
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.stepsContainer}>
            {[
              ...
              <View key={i} style={styles.stepRow}>
                ...
                <View style={[styles.stepIconWrap, { backgroundColor: C.brandBg }]}><Ionicons name={step.icon} size={20} color={C.brand} /></View>
                <View style={styles.stepInfo}><Text style={styles.stepTitle}>{step.title}</Text><Text style={styles.stepDesc}>{step.desc}</Text></View>
              </View>
            ))}
          </View>
          <View style={styles.escrowBanner}>
            <Ionicons name="shield-checkmark-outline" size={20} color={C.brandD} />
            <View style={{ flex: 1 }}><Text style={styles.escrowTitle}>Secure Escrow Payments</Text><Text style={styles.escrowText}>All payments are held securely until the buyer confirms delivery. Your money is always protected.</Text></View>
          </View>
        </View>
        ...
        <View style={styles.channelsCard}>
          ...
          <View style={styles.channelsRow}>
            {channels.map((ch) => (
              <TouchableOpacity key={ch.id} style={[styles.channelBtn, { backgroundColor: ch.bg }]} onPress={ch.onPress} activeOpacity={0.75}>
                <View style={[styles.channelIconWrap, { backgroundColor: ch.color }]}><Ionicons name={ch.icon} size={22} color="#fff" /></View>
        ...
        <View style={styles.card}>
          <View style={styles.faqHeaderRow}>
            <View style={styles.cardTitleRow}>
              <View style={styles.cardTitleIcon}><Ionicons name="chatbubble-ellipses" size={16} color={C.brand} /></View>
              <Text style={styles.cardTitle}>Frequently Asked Questions</Text>
            </View>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.topicsScroll}>
            <TouchableOpacity style={[styles.topicChip, selectedTopic === 'all' && styles.topicChipActive]} onPress={() => setSelectedTopic('all')}>
              <Text style={[styles.topicChipText, selectedTopic === 'all' && styles.topicChipTextActive]}>All</Text>
            </TouchableOpacity>
            {supportTopics.map((t) => (
              <TouchableOpacity key={t.id} style={[styles.topicChip, selectedTopic === t.id && styles.topicChipActive]} onPress={() => setSelectedTopic(t.id)}>
                <Ionicons name={t.icon} size={12} color={selectedTopic === t.id ? '#fff' : C.t2} style={{ marginRight: 5 }} />
          ...
          <View style={styles.faqList}>
            {filteredFAQs.length === 0 ? (
              <View style={styles.emptyFAQ}><Ionicons name="search-outline" size={40} color="#D1D5DB" /><Text style={styles.emptyFAQText}>No FAQs for this topic</Text><TouchableOpacity onPress={() => setSelectedTopic('all')}><Text style={styles.emptyFAQLink}>View all FAQs →</Text></TouchableOpacity></View>
            ) : (
              filteredFAQs.map((faq, idx) => (
                <View key={faq.id} style={[styles.faqItem, idx === filteredFAQs.length - 1 && { borderBottomWidth: 0 }]}>
                  <TouchableOpacity style={styles.faqRow} onPress={() => handleFAQToggle(faq.id)} activeOpacity={0.7}>
                    <View style={styles.faqNumBadge}><Text style={styles.faqNum}>{String(idx + 1).padStart(2, '0')}</Text></View>
                    <Text style={styles.faqQ}>{faq.question}</Text>
                    <Ionicons name={activeFAQ === faq.id ? 'remove' : 'add'} size={20} color={activeFAQ === faq.id ? C.brand : C.t3} />
                  </TouchableOpacity>
                  {activeFAQ === faq.id && <View style={styles.faqAnswer}><Text style={styles.faqAnswerText}>{faq.answer}</Text></View>}
                </View>
              ))
            )}
          </View>
        ...
        <View style={styles.ctaFooter}>
          <View style={styles.ctaIconRow}>
            <View style={styles.ctaIcon}><Ionicons name="headset" size={22} color={C.brand} /></View>
          </View>
          <Text style={styles.ctaTitle}>Need urgent help?</Text>
          <Text style={styles.ctaSub}>Our vendor support team is ready to assist with any issues — from orders and payments to account questions.</Text>
          <View style={styles.ctaButtons}>
            <TouchableOpacity style={styles.ctaPrimary} onPress={handleWhatsApp} activeOpacity={0.85}>
              <Ionicons name="logo-whatsapp" size={16} color="#fff" /><Text style={styles.ctaPrimaryText}>Chat on WhatsApp</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.ctaSecondary} onPress={() => handleCall('+233505671577')} activeOpacity={0.85}>
              <Ionicons name="call-outline" size={16} color={C.brand} /><Text style={styles.ctaSecondaryText}>Call Now</Text>
            </TouchableOpacity>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  topBar: { backgroundColor: C.white, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14 },
  backBtn: { padding: 4, width: 36 },
  topBarTitle: { flex: 1, textAlign: 'center', fontSize: 17, fontWeight: '700', color: C.t1, letterSpacing: 0.3 },
  scrollContent: { paddingBottom: 20 },

  hero: { backgroundColor: C.brand, borderRadius: 24, marginHorizontal: 8, marginBottom: 8, paddingTop: 32, paddingBottom: 52, alignItems: 'center', overflow: 'hidden', position: 'relative' },
  heroCircle1: { position: 'absolute', width: 200, height: 200, borderRadius: 100, borderWidth: 1, borderColor: 'rgba(255,255,255,0.07)', top: -50, right: -50 },
  heroCircle2: { position: 'absolute', width: 140, height: 140, borderRadius: 70, borderWidth: 1, borderColor: 'rgba(255,255,255,0.07)', top: 50, left: -40 },
  ...
  heroIconInner: { width: 62, height: 62, borderRadius: 31, backgroundColor: C.white, justifyContent: 'center', alignItems: 'center' },
  ...
  availDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: '#5EEAD4' },
  availText: { fontSize: 12, color: 'rgba(255,255,255,0.85)', fontWeight: '500' },

  card: { backgroundColor: C.white, marginHorizontal: 16, marginBottom: 14, borderRadius: 20, padding: 20, shadowColor: C.black, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.04, shadowRadius: 12, elevation: 3 },
  cardLabel: { fontSize: 10, fontWeight: '700', color: C.t3, letterSpacing: 1.2, marginBottom: 16 },
  cardTitleRow: { flexDirection: 'row', alignItems: 'center', gap: 10, flex: 1 },
  cardTitleIcon: { width: 30, height: 30, borderRadius: 9, backgroundColor: C.brandBg, justifyContent: 'center', alignItems: 'center' },
  ...
  stepNumber: { width: 22, height: 22, borderRadius: 11, backgroundColor: C.brand, justifyContent: 'center', alignItems: 'center' },
  stepNumberText: { color: '#fff', fontSize: 11, fontWeight: '800' },
  stepIconWrap: { width: 38, height: 38, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
  stepInfo: { flex: 1 },
  stepTitle: { fontSize: 13, fontWeight: '700', color: C.t1, marginBottom: 2 },
  stepDesc: { fontSize: 11, color: C.t2, lineHeight: 15 },

  escrowBanner: { flexDirection: 'row', gap: 12, backgroundColor: C.brandBg, borderRadius: 14, padding: 14, borderWidth: 1, borderColor: C.brandBorder, alignItems: 'flex-start' },
  escrowTitle: { fontSize: 13, fontWeight: '700', color: C.brandD, marginBottom: 3 },
  escrowText: { fontSize: 11, color: C.brandD, lineHeight: 17 },

  feeTable: { backgroundColor: '#F9FAFB', borderRadius: 14, padding: 14, marginBottom: 14 },
  feeRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#F0F0F0' },
  ...
  channelSub: { fontSize: 10, color: C.t3, textAlign: 'center' },
  ...
  topicChipActive: { backgroundColor: C.brand },
  topicChipText: { fontSize: 12, fontWeight: '600', color: C.t2 },
  ...
  faqNumBadge: { width: 28, height: 28, borderRadius: 8, backgroundColor: C.brandBg, justifyContent: 'center', alignItems: 'center' },
  faqNum: { fontSize: 10, fontWeight: '800', color: C.brandD },
  faqQ: { flex: 1, fontSize: 14, fontWeight: '600', color: C.t1, lineHeight: 20 },
  faqAnswer: { paddingLeft: 40, paddingBottom: 16, paddingRight: 4 },
  faqAnswerText: { fontSize: 13, color: C.t2, lineHeight: 21 },
  emptyFAQ: { alignItems: 'center', paddingVertical: 32 },
  emptyFAQText: { fontSize: 14, color: C.t3, marginTop: 10, marginBottom: 8 },
  emptyFAQLink: { fontSize: 13, color: C.brand, fontWeight: '600' },

  ctaFooter: { marginHorizontal: 16, marginBottom: 8, borderRadius: 22, overflow: 'hidden', backgroundColor: C.brand, padding: 28, alignItems: 'center', position: 'relative' },
  ...
  ctaSecondaryText: { fontSize: 14, fontWeight: '700', color: C.brand },
});
